...
In particular, by Chebyshev's bounds on the prime-counting function $\pi$, we have $f(N) \geq N/2 + \Omega(N/\log N)$. This improves on the previous formalised lower bound $f(N) \geq N/2 + \Omega(\sqrt{N}/\log N)$ \TODO{reference our earlier safe-primes work} by a factor of $\sqrt{N}$.
...
\begin{proof}
A pair of shape $(s, t)$ with $s < t$ and scale $m$ is contained in $[1, N]$ iff $t(s+t) m \leq N$. Summing over coprime $(s, t)$ with $s < t$ gives the displayed sum. The asymptotic $\Theta(N \log N)$ follows from $\sum_{(s,t) \text{ coprime}} 1/(t(s+t)) = \Theta(\log T)$ when truncated at $t \leq T$, integrated against $N/(t(s+t))$. \TODO{Sketch the divergent-sum estimate more carefully.}
\end{proof}
...
\emph{(iii) $(2p, m)$ for $m$ odd is not a pair.} The only odd pair-partner of $2p$ is $a = p(p - 2)$ (computed below). For $p > \sqrt{N} + 1$, $a = p(p-2) > (\sqrt{N} + 1)(\sqrt{N} - 1) = N - 1$, hence $a \geq N$, but in fact one checks $a > N$ unless equality. \TODO{tighten boundary; require strict $p > 1 + \sqrt{1 + N}$.}
...
\begin{remark}[Comparison with previous lower bound]
The previous formal lower bound \TODO{reference our safe-primes work} achieved $\Omega(\sqrt{N}/\log N)$ via the family $\{2^{\alpha} \cdot p : p \text{ safe prime}, \alpha \geq 1\}$ for small primes $p \leq \sqrt{N/2}$. The new construction $\{2p : p > \sqrt{N}\}$ takes \emph{large} primes (each contributing one element) rather than \emph{small} primes (each contributing $\log N$ elements but only $\sqrt{N}/\log N$ small safe primes exist). The two contributions are essentially complementary, but the new one is asymptotically much larger.
\end{remark}
...
\begin{proof}
\TODO{Carefully use the parametrisation. Cross-parity pairs are exactly shape-$(s, t)$ pairs with $s + t$ odd (so $s, t$ have different parities). For each such shape, count instances $(s(s+t)m, t(s+t)m)$ with the odd element $\leq N$ and even element $\leq N$. The shape $(1, 2)$ contributes $N/6$ pairs (and $(s, t) = (1, 4)$ contributes $N/20$, etc.). Sum over shapes diverges like $\log T$ at truncation $t \leq T$.}
\end{proof}

\subsection{Conditional structure theorem}

\begin{corollary}\label{prop:structure-corollary}
Let $A \subseteq [1, N]$ be pair-free with $|A_e| \geq \alpha N$ for some $\alpha > 0$. Then there exists $C = C(\alpha) > 0$ such that
\[
  |A_o| \leq \lceil N/2 \rceil - C \log N.
\]
Consequently, $|A| \leq N/2 + \alpha N - C \log N$.
\end{corollary}

\begin{proof}
By Theorem~\ref{thm:double-counting}, $\sum_{e \in A_e} |P_o(e)| \leq |A_e| \cdot (\lceil N/2 \rceil - |A_o|)$. The left side is at least $\alpha \cdot \Theta(N \log N)$ \emph{on average}: more precisely, the average of $|P_o(e)|$ over $e \in [1, N/2] \cap 2\Z$ is $\Theta(\log N)$ by Theorem~\ref{thm:Po-sum}, and a positive density subset $A_e$ inherits at least a fraction of this average. \TODO{Make the second-moment / Cauchy-Schwarz step rigorous.} Hence $|A_e| \cdot \Omega(\log N) \leq |A_e| \cdot (\lceil N/2 \rceil - |A_o|)$, giving $|A_o| \leq \lceil N/2 \rceil - \Omega(\log N)$.
\end{proof}

\begin{remark}
Corollary~\ref{prop:structure-corollary} is too weak to close the conjecture: it gives only $|A| \leq N/2 + \alpha N - O(\log N)$, whereas Conjecture~\ref{conj:half} demands $|A| \leq N/2 + o(N)$. The shortfall is exactly $\alpha N$ --- the cross-parity inequality fails to constrain $|A_e|$ itself.

The next sections explore whether the $|A_e|$ constraint can be obtained from other considerations.
\end{remark}
...
The empirical density $f(N)/N$ is slowly decreasing. Log-log regression of the gap $g(N) := f(N) - \lceil N/2 \rceil$ on $N$ gives
\[
...
\TODO{Reference the ILP/SAT scripts and explain the bijection $A \subseteq \text{even} \leftrightarrow A/2 \subseteq [1, N/2]$ with modified pair condition $(a + b) \mid 2ab$. The modified condition has only marginally more solutions than the unmodified one, hence $f_{\text{even}}(N) \approx f(N/2)$.}
\end{proof}
...
\TODO{Add discussion of the asymptotic density-count gap: the structural three-family inequality is formalized, but the density count $|D_U| = (1/7)(N/30) + o(N)$ is currently in the informal docstring only, paralleling the situation in the existing VanDoorn formalization. A full Lean formalization of the asymptotic constants $25/28$ and $373/420$ would require formalizing the VDParam density argument, which is not currently in Mathlib.}
