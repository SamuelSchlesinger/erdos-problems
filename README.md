...
| [313](https://www.erdosproblems.com/313) | Primary pseudoperfect numbers | Statement formalized; successor-prime extension; examples 2, 6, 42, 1806; every example > 2 is pseudoperfect; equivalence with the literature definition `m squarefree ∧ ∑_{p∣m} 1/p + 1/m = 1`, derived from `Finset.squarefree_prod_of_pairwise_isCoprime` and `Nat.prod_primeFactors_invOn_squarefree` |
...
| [417](https://www.erdosproblems.com/417) | Totient value ratio | Statement formalized via the finite counting functions `V'(x)` and `V(x)`; the basic inclusion `V'(x) ≤ V(x)` is proved, together with positivity of both counts for every `x ≥ 1` |
| [431](https://www.erdosproblems.com/431) | Inverse Goldbach problem | Statement formalized on subsets of `ℕ`; eventual agreement of a sumset `A + B` with the primes is packaged; elementary parity obstructions show this is impossible if both sets have infinitely many even elements, or both have infinitely many odd elements |
...
| [301](https://www.erdosproblems.com/301) | Unit fraction sum-free sets | Upper half construction, bridge to #302, dedicated van Doorn 25/28 upper bound, same-signature 145/168 packing improvement, dense-template 163/195 candidate search, weighted and parametric certificate bridges, multiplier-fiber reduction, pseudoperfect/divisor obstruction bridge to #470 |
...
| [893](https://www.erdosproblems.com/893) | Divisors of Mersenne numbers | Statement formalized; positivity of each divisor-count term, exact successor recurrence, monotonicity of partial sums, the trivial bound `f(n) ≥ n`, the divisor-injection bound `τ(k) ≤ τ(2^k − 1)` via `d ∣ k ⇒ (2^d − 1) ∣ (2^k − 1)`, the structural improvement `f(n) ≥ ∑_{k=1}^{n} τ(k)`, the explicit numeric consequence `f(n) ≥ 2n − 1`, and the closed-form harmonic lower bound `f(n) ≥ n · H_n − n` obtained via Dirichlet's hyperbola identity (an `Θ(n log n)` lower bound) |
...
| [950](https://www.erdosproblems.com/950) | Reciprocal sums over prime gaps | Statement formalized; nonnegativity, exact vanishing for `n = 0, 1, 2`, the first positive lower bound coming from the prime `2`, the pointwise upper bound `f(n) ≤ H_{n-1}` via the injection `p ↦ n - p` on prime gaps, the per-prime lower bound `f(p+1) ≥ 1`, and the consequence that `f(n) ≥ 1` for arbitrarily large `n` (first partial step toward `limsup f = ∞`) |
| [195](https://www.erdosproblems.com/195), [196](https://www.erdosproblems.com/196), [197](https://www.erdosproblems.com/197) | Monotone arithmetic progressions in permutations | First formalization of this cluster. Davis–Entringer–Graham–Simmons base case proved: every permutation of `ℕ` contains a monotone 3-term AP. Structural necessary condition: a 3-free set contains no infinite AP. Density layer + the LeSaulnier–Vijay reduction: if every 3-free set has upper density `≤ uA`, lower density `≤ uB`, and `uA+uB<1`, then the Erdős–Graham partition (#197) is impossible (the precise open target). Van der Corput theorem: the reverse-binary order on `ℕ` has no monotone 3-AP (2-adic argument; the engine for 3-free constructions) |
...
Erdos/
  ...
  InverseGoldbach/         -- #431: can a sumset A + B eventually equal the primes?
  ...
  TotientValueRatio/       -- #417: compare two counting functions for totient values
