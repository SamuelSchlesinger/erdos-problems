...
| [893](https://www.erdosproblems.com/893) | Divisors of Mersenne numbers | Statement formalized; positivity of each divisor-count term, exact successor recurrence, monotonicity of partial sums, the trivial bound `f(n) ≥ n`, the divisor-injection bound `τ(k) ≤ τ(2^k − 1)` via `d ∣ k ⇒ (2^d − 1) ∣ (2^k − 1)`, the structural improvement `f(n) ≥ ∑_{k=1}^{n} τ(k)`, and the explicit numeric consequence `f(n) ≥ 2n − 1` |
