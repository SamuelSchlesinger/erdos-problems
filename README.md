...
| [Erdős-Moser](https://en.wikipedia.org/wiki/Erdos-Moser_equation) | Power-sum Diophantine equation | `k=1` uniqueness, odd-exponent exclusion (`Odd k ∧ 1<k` gives no solutions), reduction of all non-classical cases to even branch (`Even k ∧ 8 ≤ k`), no solutions for `k=2..7`, modular obstructions (`m ≡ 0/3 mod 4`; odd `k` gives `m ≡ 0 mod 3` and `m ≡ 0/3 mod 12`; odd `k≥3` gives `m ≡ 0 mod 8` and `m ≡ 0 mod 24`; mod-5 split: `k ≡ 1 (mod 4)` gives `m ≡ 0/3 mod 5`, `k ≡ 3 (mod 4)` gives `m ≡ 0 mod 5`, `k ≡ 2 (mod 4)` gives `m ≡ 0 mod 5`; `k ≡ 2 (mod 6)` gives `m ≡ 0 mod 7`; `k ≡ 2 (mod 10)` gives `m ≡ 0 mod 11`; `k ≡ 2 (mod 12)` gives `m ≡ 0 mod 35`, `m ≡ 0 mod 13`, hence `m ≡ 0 mod 455`, and with mod-4 gives `m ≡ 0/35 mod 140` and `m ≡ 0/455 mod 1820`; `k ≡ 2 (mod 60)` gives `m ≡ 0 mod 5005`, and with mod-4 gives `m ≡ 0/15015 mod 20020`; with odd `k≥3`, this yields `m ≡ 0/48 mod 120` in the `k ≡ 1` branch and `m ≡ 0 mod 120` in the `k ≡ 3` branch), bounded certificate (`m≤150`,`k≤25`) |
