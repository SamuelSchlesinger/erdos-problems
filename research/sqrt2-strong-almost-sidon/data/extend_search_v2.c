/*
 * extend_search_v2.c — strong almost-Sidon (SAS) extremizer search,
 * enumerating ALL extremizers at the maximum size and recording the
 * multiplicity invariant 2*r_A(n*) - |A| for each.
 *
 * Based on extend_search.c. Differences:
 *
 *   - Two-phase: first determine best_size, then enumerate ALL sets of that size.
 *     Phase 1 uses the same backtrack as before; Phase 2 prunes only when
 *     cur_size + (N - start) < best_size  (strictly less), to ensure all
 *     equally-best sets are kept.
 *   - For each maximum set found, computes exception value n*, multiplicity
 *     r_A(n*), and the deviation 2*r_A(n*) - |A|.
 *   - Outputs CSV-like lines:
 *        N <tab> |A| <tab> n* <tab> r(n*) <tab> 2r-|A| <tab> a1,a2,...
 *
 * Compile: cc -O3 -march=native -funroll-loops -o extend_search_v2 extend_search_v2.c
 */
...
/* For enumeration phase */
static int enum_mode;        /* 0 = find best_size; 1 = enumerate all */
static int n_extremizers;
static int extremizer_size;  /* fixed once best_size known */
static int (*extremizers)[MAXN + 2];
...
static void backtrack(int d, int start) {
    ...
    if (enum_mode == 0) {
        ...
        /* Enumeration mode: collect all sets of size == extremizer_size */
        if (cur_size == extremizer_size) {
            if (n_extremizers < MAX_EXTREMIZERS) {
                memcpy(extremizers[n_extremizers], stk_elts[d], cur_size * sizeof(int));
                n_extremizers++;
            }
            return;  /* can't add more without exceeding */
        }
        if (cur_size + (N - start) < extremizer_size) return;
    }
    ...
    for (int x = start + 1; x <= N; x++) {
        if (enum_mode == 0) {
            ...
            if (cur_size + 1 + (N - x) < extremizer_size) break;
        }
...
/* Compute (n*, r_A(n*)) where n* is the most-frequent pair-sum and
 * r_A(n*) is # unordered pairs {a,b} with a+b = n*. If all sums unique,
 * returns n* = 0, r = 0.  In SAS, at most one value has r >= 2. */
static void compute_exception(const int *A, int sz, int *nstar, int *r) {
    /* Pair-sum range: [2*A[0], 2*A[sz-1]] but we just allocate based on N */
...
static void enumerate_all(int n) {
    N = n;
    n_words = (2 * N + 64 + 1) / 64;
    bs_zero(stk_A[0]);
    bs_zero(stk_once[0]);
...
int main(int argc, char **argv) {
    int n_start = 80, n_end = 100;
    int do_enum = 0;
    if (argc >= 2) n_start = atoi(argv[1]);
    if (argc >= 3) n_end = atoi(argv[2]);
    if (argc >= 4) do_enum = atoi(argv[3]);

    extremizers = (int (*)[MAXN + 2])malloc(sizeof(int) * (MAXN + 2) * MAX_EXTREMIZERS);

    for (int n = n_start; n <= n_end; n++) {
        clock_t t0 = clock();
        int f = solve(n);
        double dt1 = (double)(clock() - t0) / CLOCKS_PER_SEC;

        int n_ext = 1;
        if (do_enum) {
            clock_t t1 = clock();
            enumerate_all(n);
            double dt2 = (double)(clock() - t1) / CLOCKS_PER_SEC;
            n_ext = n_extremizers;
            printf("# N=%d size=%d  t_find=%.2fs t_enum=%.2fs  n_extremizers=%d\n",
                   n, f, dt1, dt2, n_ext);
        } else {
            printf("# N=%d size=%d  t=%.2fs\n", n, f, dt1);
        }
        fflush(stdout);
        ...
        if (do_enum && n_ext > 0) {
            for (int e = 0; e < n_ext; e++) {
                int *A = extremizers[e];
                int nstar, r;
                compute_exception(A, f, &nstar, &r);
                int inv = 2 * r - f;
                printf("%d\t%d\t%d\t%d\t%d\t", n, f, nstar, r, inv);
                for (int i = 0; i < f; i++) {
                    printf("%d%s", A[i], i + 1 < f ? "," : "");
                }
                printf("\n");
            }
        } else {
            /* Just print the one found */
            int nstar, r;
            compute_exception(best_set, f, &nstar, &r);
            int inv = 2 * r - f;
            printf("%d\t%d\t%d\t%d\t%d\t", n, f, nstar, r, inv);
            for (int i = 0; i < f; i++) {
