...
| 2026-05-22 | **Attempt 18 (game-theoretic / Builder-Saboteur) negative**: framing `f(N)` as a 2-player game gives clean restatement but no bound. Symmetry recovers EF; Yao/cyclic randomization doesn't transfer to `[1,N]`; competitive ratio is relative (`g/f`), not absolute; random potential functions collapse to Pikhurko-style Fourier (already attempted). Residual observation: empirical "trapping" of near-EF Builder prefixes restates the rigidity conjecture dynamically — no easier to prove. Detail in `game-theoretic-attack.md`. |
